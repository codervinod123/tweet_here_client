...
const PostCard = ({ tweet }) => {
  ...
  const sendCommentRequest = async (e, modelId) => {
    ...
    const databaseURL = import.meta.env.VITE_BACKEND_URL;
    const token = localStorage.getItem("token");
    const response = await axios.post(`${databaseURL}/api/v1/tweet/comment`, { "commentItem": comment }, {
      headers: {
        modelName: "Tweet",
        modelId: modelId,
        token: token,
        ...
        <div className="">
          ...
          <div className="flex gap-2">
            ...
            <form className="w-full rounded flex px-4 bg-[#ECFFE6]">
              ...
              ></textarea>
              <button onClick={(e) => sendCommentRequest(e, tweet._id)} type="submit" className="">
