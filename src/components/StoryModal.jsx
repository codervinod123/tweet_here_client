...
const StoryModal = ({ reference, stories, page, setPage }) => {
  ...
  return (
    <div
      onClick={handleClose}
      className="fixed inset-0 flex justify-center items-center transition-colors m-8 bg-gray-200/90"
    >
      ...
      <div onClick={(e) => e.stopPropagation()} className="mx-8 my-8">
        <div className="flex gap-4 justify-between items-center">
          <button
            onClick={prevStory}
            className="bg-gray-400 p-2 rounded-full text-white font-bold cursor-pointer flex items-center justify-center hover:bg-gray-400/70 transition-all duration-500"
          >
            <GrPrevious />
          </button>
          <div>
                ...
                <React.Fragment key={index}>
                  {page == index && (
                    <div className="flex justify-center flex-col gap-4 rounded-md">
                      ...
                      <div>
                        <img
                          className="rounded w-96 max-h-[75vh]"
                          src={story.content}
                          alt={story.content}
                        />
                      </div>
